...
res <- rma.mv(Com_yi,Com_vi,random = ~1|Level4/Level3/Level2,method = 'ML',data = dataMatrix)
##==== Random effects model ====##

##==== Plot size ====##
res_1 <- rma.mv(Com_yi,Com_vi,mods = ~PlotSize,random = ~1|Level4/Level3/Level2,method = 'ML',data = dataMatrix)
modelCompare_1 <- anova.rma(res,res_1)
##==== Plot size ====##

##==== Output ====##
ouputFileName <- paste0(currentPath,'/Results/Community_Height_Com.txt')
