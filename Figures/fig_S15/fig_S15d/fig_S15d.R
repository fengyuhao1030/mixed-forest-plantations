...
##==== Part 2: Fit PRE ====##
res_Net <- rma.mv(Net_yi,Net_vi,mods = ~PRE,random = ~1|Level4/Level3/Level2,method = 'ML',data = dataMatrix)
res_Com <- rma.mv(Com_yi,Com_vi,mods = ~PRE,random = ~1|Level4/Level3/Level2,method = 'ML',data = dataMatrix)
res_Sel <- rma.mv(Sel_yi,Sel_vi,mods = ~PRE,random = ~1|Level4/Level3/Level2,method = 'ML',data = dataMatrix)
# Fit line Net
...
##==== Part 3: Draw ====##
fig_S15d <- ggplot()+
  geom_point(newDataMatrix,mapping = aes(x = PRE,y = Net_yi),
  ...
  xlab('PRE')+
  theme_custom()
outputFileName <- paste0(currentPath,'/fig_S15d.pdf')
